...
import io.github.locl95.smashtools.SmashggAuth
...
class SmashggRoutesSpecs extends CatsEffectSuite{
  ...
  val testContext: Resource[IO, SmashggRoutes[IO]] = TestHelper.ContextTest
  ...
  test("Authed POST /tournaments/<tournament> should insert a tournament into tournaments migration") {
    testContext.use{
      ...
      } yield assertEquals(responseOkInsert, true)
    }
    ...
    testContext.use {
      router =>
        for {
          ...
          respOk <- TestHelper.checkF[List[Sets]](resp, Status.Ok, Some(TestHelper.sets))
        } yield assertEquals(respOk, true)
    }
  }
}

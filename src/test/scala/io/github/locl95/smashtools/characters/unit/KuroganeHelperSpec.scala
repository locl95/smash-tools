...
class KuroganeHelperSpec extends CatsEffectSuite {
  test("I can calculate which moves from one character can punish a move on shield") {
    val moveToPunish: KuroganeCharacterMove = KuroganeCharacterMove("Jab 1", Some(-10), "ground", Some(3))

    val availableMoves: List[KuroganeCharacterMove] = List(
      KuroganeCharacterMove("Jab 1", Some(-12), "ground", Some(2)),
      KuroganeCharacterMove("Forward Tilt", Some(-15), "ground", Some(5)),
      KuroganeCharacterMove("USmash", Some(-30), "ground", Some(11)),
      KuroganeCharacterMove("Neutral Air", Some(-3), "aerial", Some(3)),
      KuroganeCharacterMove("Forward Air", Some(-2), "aerial", Some(5)),
      KuroganeCharacterMove("Standing Grab", None, "ground", Some(6)),
      KuroganeCharacterMove("Timber Axe", None, "special", Some(6)),
      KuroganeCharacterMove("USmash", None, "ground", Some(9))
    )
